...
export default function Login() {
  ...
  function onSubmit(e) {
    ...
    login({ email: form.email.trim(), createdAt: new Date().toISOString() });
    navigate('/profile');
  }
  ...
  return (
    <section className="mx-auto max-w-md px-4 py-12 md:py-16">
      <div className="rounded-3xl border border-white/10 bg-white/5 p-7 shadow-soft">
        <div className="text-sm font-semibold tracking-wide text-gold-300">Login</div>
        <h1 className="mt-2 text-2xl font-semibold text-slate-50">Welcome back</h1>
        <p className="mt-2 text-sm text-slate-300">Sign in to manage your profile and orders.</p>
        ...
        <div className="mt-6 text-sm text-slate-300">
          New here?{' '}
          <Link to="/register" className="font-semibold text-gold-300 hover:text-gold-200 transition">
            Create an account
          </Link>
        </div>
      </div>
